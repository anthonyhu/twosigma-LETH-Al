...
# To correct as the method is wrong for now
if add_diff_ft:
...
#generation of linear models, picked those correlated to y
...
index2use = train[abs(train.y) < 0.075].index

for elt in cols2fit:
    print("fitting linear model on ", elt)
    model = recurrent_linear_approx(quant=.99, limit_size_train=.9)
    model.fit(train.ix[index2use, [elt]], train.ix[index2use, 'y'])
    models.append(model)
    columns.append([elt])
...
while True:
    ...
    for ind,elt in enumerate(tokeepmodels):
        pred['y']+=selected_prediction[:,ind]* (elt.predict(test[tokeepcolumns[ind]]).clip(-0.075, 0.075))
